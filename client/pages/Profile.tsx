...
const STYLE_META: Record<string, { label: string; desc: string; icon: React.ReactNode; color: string; particle: string }> = {
  competitive: {
    label: "Competitive",
    desc: "High-speed sessions · Action-heavy · Replay addict",
  ...
  casual: {
    label: "Casual",
    desc: "Long chill sessions · Easy picks · Relaxed pace",
  ...
  explorer: {
    label: "Explorer",
    desc: "Tries everything · Loves discovery · Genre hopper",
...
export default function Profile() {
  ...
  const styleMeta = STYLE_META[profile.playStyle] ?? STYLE_META.explorer;
  const engagementScore = Math.min(100, Math.round((profile.sessionsCount / 20) * 100));

  const aiInsights = [
    profile.sessionsCount > 0 && `${profile.sessionsCount} sessions logged`,
    topInterests[0] && `Top genre: ${topInterests[0][0]}`,
    profile.totalPlayTime > 0 && `${formatTime(profile.totalPlayTime)} total playtime`,
    topInterests.length >= 3 && `${topInterests.length} genres explored`,
    ...
    <div className="min-h-screen bg-[#05020f] overflow-x-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 space-y-6">
            ...
            <div className="relative px-6 sm:px-8 py-8 flex flex-col sm:flex-row sm:items-center gap-6">
              ...
              <div className="relative shrink-0">
                <motion.div
                  ...
                  style={{ background: `linear-gradient(135deg, ${styleMeta.color.replace("from-", "").replace(" via-", ", ").replace(" to-", ", ")})` }}
                  animate={{ rotate: 360 }}
                  transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
                />
                <div className="relative w-20 h-20 rounded-full bg-[#0c0618] border-2 border-white/10 flex items-center justify-center m-[3px]">
                  <div className={`w-14 h-14 rounded-full bg-gradient-to-br ${styleMeta.color} flex items-center justify-center shadow-2xl`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-violet-500/20 border border-violet-500/30 text-violet-300 text-[10px] font-black uppercase tracking-widest mb-2">
                  <Brain className="w-3 h-3" /> AI Profile · Active
                </div>

                <h1 className="text-2xl sm:text-3xl font-black text-white font-outfit mb-1 truncate">
                  {user?.msisdn}
                </h1>

                <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-gradient-to-r ${styleMeta.color} text-white text-xs font-black shadow-lg mb-2`}>
                  {styleMeta.icon}
                  {styleMeta.label} Player
                </div>
                <p className="text-white/40 text-xs">{styleMeta.desc}</p>

                {/* insight chips */}
                {aiInsights.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {aiInsights.map((t) => (
                      <InsightChip key={t} text={t} icon={<Cpu className="w-3 h-3" />} />
                    ))}
                  </div>
                )}
              </div>
          ...
          >
            {[
              { icon: <Gamepad2 className="w-5 h-5" />, label: "Sessions", value: profile.sessionsCount, color: "text-violet-400", bg: "from-violet-500/20 to-violet-600/5", border: "border-violet-500/20" },
              { icon: <Clock    className="w-5 h-5" />, label: "Play Time", value: formatTime(profile.totalPlayTime), color: "text-fuchsia-400", bg: "from-fuchsia-500/20 to-fuchsia-600/5", border: "border-fuchsia-500/20" },
              { icon: <Activity className="w-5 h-5" />, label: "Engagement", value: `${engagementScore}%`, color: "text-orange-400", bg: "from-orange-500/20 to-orange-600/5", border: "border-orange-500/20" },
              { icon: <TrendingUp className="w-5 h-5" />, label: "Genres", value: topInterests.length, color: "text-cyan-400", bg: "from-cyan-500/20 to-cyan-600/5", border: "border-cyan-500/20" },
            ].map((s, i) => (
              <motion.div
                key={s.label}
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-sm font-extrabold text-white font-outfit">AI Interest Graph</h2>
                    <p className="text-[10px] text-white/30">Built from your play behavior</p>
                  </div>
                </div>
              ...
              <div className="px-6 py-5 space-y-4">
                {topInterests.map(([cat, score], i) => {
                  ...
                  const isHovered = hoveredBar === cat;
                  return (
                    <motion.div
                      key={cat}
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${meta?.color ?? "from-violet-500 to-fuchsia-500"} flex items-center justify-center text-white shadow-lg ${isHovered ? meta?.glow ?? "" : ""} transition-shadow`}>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {i === 0 && (
                            <span className="px-1.5 py-0.5 rounded-md bg-amber-500/20 text-amber-400 text-[9px] font-black uppercase">Top</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="relative h-2 rounded-full bg-white/5 overflow-hidden">
                        <motion.div
                          className={`absolute inset-y-0 left-0 rounded-full bg-gradient-to-r ${meta?.color ?? "from-violet-500 to-fuchsia-500"}`}
                          initial={{ width: 0 }}
                          animate={{ width: `${pct}%` }}
                          transition={{ duration: 1, delay: 0.35 + i * 0.1, ease: "easeOut" }}
                        />
                        {/* shimmer */}
                        <motion.div
          ...
          >
            ...
            <div className="rounded-3xl bg-white/[0.03] border border-white/8 px-6 py-6 flex items-center gap-5">
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-sm font-extrabold text-white font-outfit">Engagement Score</span>
                </div>
                <p className="text-xs text-white/40 leading-relaxed">
                  {engagementScore < 30
                    ? "Just getting started — play more to unlock full AI personalization"
                    : engagementScore < 70
                    ? "Your AI profile is building up nicely"
                    : "Highly engaged — AI recommendations are fully tuned to you"}
                </p>
              </div>
            </div>
            ...
            <div className="rounded-3xl bg-white/[0.03] border border-white/8 px-6 py-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-extrabold text-white font-outfit">AI Engine Status</span>
              </div>
              <div className="space-y-2.5">
                {[
                  { label: "Behavior Tracking", active: true },
                  { label: "Interest Modeling", active: profile.sessionsCount >= 1 },
                  { label: "Recommendations", active: profile.sessionsCount >= 3 },
                  { label: "Play Style Detection", active: profile.sessionsCount >= 3 },
                ].map((item) => (
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-sm font-extrabold text-white font-outfit">Recently Played</h2>
                    <p className="text-[10px] text-white/30">{recentlyPlayed.length} games in history</p>
                  </div>
                </div>
              </div>
          ...
          {profile.sessionsCount === 0 && (
            ...
            >
              ...
              <h3 className="text-xl font-black text-white mb-2 font-outfit">AI Profile Initializing…</h3>
              <p className="text-white/40 mb-7 text-sm max-w-xs mx-auto">
                Play 3 games and your AI engine will activate — personalized recommendations, interest graph, and play style detection.
              </p>
